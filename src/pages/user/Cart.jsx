...
export default function Cart() {
    ...
    const totalQuantity = selected.reduce((sum, p) => sum + p.quantity, 0);
    const totalPrice = selected.reduce((sum, p) => sum + p.quantity * p.data.price, 0);
    const handleDelete = async (product_id) => {
        ...
        try {
            ...
            await axios.delete(`http://163.223.211.23:3000/cart/${userId}/items/${product_id}`, {
                headers: { Authorization: `Bearer ${token}` }
            });

            alert("Xóa sản phẩm thành công!");
            // TODO: Cập nhật lại danh sách sản phẩm nếu cần
            fetchCountCart();
        } catch (err) {
